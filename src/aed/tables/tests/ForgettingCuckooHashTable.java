...
public class ForgettingCuckooHashTable<Key,Value> implements ISymbolTable<Key,Value> {
    ...
    public void put(Key k, Value v) 
    {
        ...
        if (getLoadFactor() >= 0.5f) resizeTable(true);
        ...
        if (sameHashErro(entry, table0[hash0], table1[hash1])) 
            throw new IllegalArgumentException("Three Keys with the same hashcode");

        while (true) 
        {
            if (maxswapsCountReached(entry)) resizeTable(true);
            swapLogic(entry);
    ...
    private void reHash(KeyValueEntry<Key,Value> entry)
    {
    ...
    private boolean sameHashErro(KeyValueEntry<Key, Value> entry, KeyValueEntry<Key, Value> entry0, KeyValueEntry<Key, Value> entry1) 
    {
    ...
    public void resizeTable(boolean isIncreasing) 
    {
        ...
        if (isIncreasing) primordial++;
        if (!isIncreasing && primordial > 0) primordial--;

        int newcapacitytable0 = primesTable0[primordial];
        int newcapacitytable1 = primesTable1[primordial];
    ...
    private void swapLogic(KeyValueEntry<Key, Value> entry) 
    {
        ...
        if (keySwapLogging) 
        {
            ...
            if (entry.swapsCount == -1) 
            {
                ...
                entry.indiexRec = index;
                keySwapIndex++;
            } else {
                if (keySwapRecords[entry.indiexRec % 100 ] == entry) 
                {
                    entry.swapsCount++;
                } else {
                    entry.swapsCount++;
                    keySwapRecords[index] = entry;
                    entry.indiexRec = keySwapIndex % 100;
                    keySwapIndex++;
                }
            }
        }
    ...
    private void forgottenSwapLogic(KeyValueEntry<Key, Value> entry, KeyValueEntry<Key, Value> forgottenEntry) 
    {
        ...
        int index = forgottenEntry.indiexRec % 100;
